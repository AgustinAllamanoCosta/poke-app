...
export class CardsController {
  ...
  @ApiResponse({
    status: 201,
    description: 'A list of pokemon card',
    type: [PokeCardDTO],
  })
  @ApiResponse({ status: 401, description: 'Not Authorized' })
  @ApiResponse({ status: 500, description: 'App can not connect to the db' })
  public async returnAllCards(
    @Param('id') id: string,
  ): Promise<Array<PokeCardDTO>> {
  ...
  @ApiResponse({
    status: 201,
    description: 'The card maching with the id',
    type: PokeCardDTO,
  })
  @ApiResponse({ status: 401, description: 'Not Authorized' })
  @ApiResponse({ status: 500, description: 'App can not connect to the db' })
  public async returnACard(
    @Param('id') id: string,
  ): Promise<PokeCardDTO | null> {
  ...
  @ApiOkResponse({
    description: 'List of cards weakness and resistance',
    type: CardRivalsDTO,
  })
  @ApiResponse({ status: 401, description: 'Not Authorized' })
  @ApiResponse({ status: 500, description: 'App can not connect to the db' })
